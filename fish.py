...
class Player:
    def __init__(self, x, y, size, controls, name, sprite):
        ...
        self.dead = False
        self.name = name
        self.sprite = pygame.transform.scale(sprite, (int(size * 2), size))
...
class Fish:
    def __init__(self, x, y, size, player_size, sprite):
        ...
        self.angle = 0
        self.sprite = pygame.transform.scale(sprite, (int(size * 2.5), int(size * 1.25)))
        self.flipped_sprite = pygame.transform.flip(self.sprite, True, False)
    ...
    def move(self, other_fish, players):
        ...
        for entity in other_fish + players:
            if entity != self and entity.size > self.size * 1.2:
    ...
    def flee(self, other_fish, players):
        ...
        nearest_threat = min(
            (threat for threat in threats if threat != self and threat.size > self.size * 1.2),
    ...
    def find_target(self, other_fish, players):
        ...
        for target in potential_targets:
            if target != self and self.size > target.size * 1.2:
                dx = target.x - self.x
                dy = target.y - self.y
                dist = math.hypot(dx, dy)
...
def spawn_fish(player_size):
    size = random.randint(10, int(player_size * 1.2))
...
while running:
    ...
    if not game_over:
        ...
        for fish in other_fish[:]:
            ...
            for other in other_fish[:]:
                if fish != other and math.hypot(fish.x - other.x, fish.y - other.y) < (fish.size + other.size) / 2:
                    if fish.size > other.size * 1.2:
                        fish.size += 0.25
                        other_fish.remove(other)
                        fish.reset_chase()
                        break
        ...
        # Spawn new fish
        if len(other_fish) < 50 and random.random() < 0.05:
            other_fish.append(spawn_fish(max(player.size for player in players if not player.dead)))
